						...
						if(!empty($_GET['param'])){
							...
							if(!empty($p->token)){
									...
									$this->result = $this->mongo_db->select(array(
										'money_transfer','date_create','fee','total_transfer',
										'balancer_clients','beneficiary_balancer','balancer_plus',
										'balancer_munis','payer_balancer','payer_id','beneficiary_id',
										'beneficiary','payer_name','client_id','client_name',
										'password_transfer','reseller','transaction',
										'bank_name','account_holders','bank_account',
										'provinces_bank','branch_bank','type',
										'action','date_update_transfer','transfer_transaction',
									))->where(array('_id' => new \MongoId($p->keys),'client_id'=>$p->client_id))->get('withdrawal');
